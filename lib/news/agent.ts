...
export async function fetchAndSummarizeNews(): Promise<Omit<NewsItem, "id" | "created_at">[]> {
  ...
  return Promise.all(selected.map(async (story, index) => {
    ...
    try {
      const bilingual = await openAI(`Bu teknoloji haberini iki dilde, öğretici ve bağlamı güçlü biçimde özetle. Yalnızca geçerli JSON döndür: {"tr":"Türkçe özet", "en":"English summary"}. Sabit bir kelime veya cümle sınırı kullanma; özetin uzunluğuna makalenin içeriği ve karmaşıklığı karar versin. Makalenin amacını ve ana fikrini açıkça anlat, dolgu ve tekrarları at, önemli teknik kavramları gerektiği kadar açıkla ve olası etkileri belirt. Tek cümlelik, yüzeysel veya aşırı kısa özet üretme; ancak gereksiz ayrıntılarla makaleyi yeniden yazma. Tarafsız ol, başlık/madde işareti/kaynak ekleme ve metinde olmayan ayrıntıları uydurma.\nBaşlık: ${story.title}\nMakale metni:\n${sourceTexts[index] || "Makale metni alınamadı; başlıktan güvenli ve temkinli bir özet çıkar."}`);
