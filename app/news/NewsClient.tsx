...
export function NewsCards({ items }: { items: NewsItem[] }) {
  ...
  return <div className="space-y-3">{items.map((item) => { const open = openId === item.id; return <article key={item.id} className={`rounded-xl border bg-white/60 dark:bg-white/[0.03] transition-colors ${open ? "border-slate-400 dark:border-slate-500" : "border-slate-200 dark:border-white/10 hover:border-slate-400 dark:hover:border-slate-500"}`}><button type="button" onClick={() => setOpenId(open ? null : item.id)} aria-expanded={open} className="w-full text-left p-5 sm:p-6"><div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400 mb-3"><time dateTime={item.published_at ?? item.created_at}>{new Intl.DateTimeFormat(language === "tr" ? "tr-TR" : "en-US", { dateStyle: "medium" }).format(new Date(item.published_at ?? item.created_at))}</time><span>·</span><span>{copy.hn}</span><span className="ml-auto text-lg leading-none text-slate-400 transition-transform duration-300">{open ? "−" : "+"}</span></div><h2 className="text-xl font-medium text-slate-900 dark:text-gray-100 leading-snug">{item.title}</h2>{open && <div className="mt-4 border-t border-slate-200 dark:border-white/10 pt-4"><p className="text-sm leading-7 text-slate-600 dark:text-gray-400">{item.summary}</p><div className="mt-4 flex flex-wrap gap-4"><a href={item.hn_url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-700 dark:text-slate-200 hover:underline">{copy.hn}<ExternalLink size={14} /></a><a href={item.article_url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-700 dark:text-slate-200 hover:underline">{copy.original}<ExternalLink size={14} /></a></div></div>}</button></article>; })}</div>;
}
